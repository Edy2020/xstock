<x-app-layout :pageTitle="'Historial de Actividad'">
    ...
    <form method="GET" action="{{ route('historial.index') }}" id="form-filters" style="display:flex; align-items:center; gap:10px; margin-bottom:16px; flex-wrap:wrap">
        ...
        <select name="modulo" class="form-select" style="flex: 1 1 120px; padding:7px 11px" onchange="document.getElementById('form-filters').submit()">
        ...
        <select name="accion" class="form-select" style="flex: 1 1 120px; padding:7px 11px" onchange="document.getElementById('form-filters').submit()">
        ...
        <input type="date" name="date" value="{{ request('date') }}" class="form-input" style="flex: 1 1 120px; padding:7px 11px" onchange="document.getElementById('form-filters').submit()">
        
        @if(request()->anyFilled(['search', 'modulo', 'accion', 'date']))
            <a href="{{ route('historial.index') }}" class="btn btn-secondary btn-sm" style="flex: 0 0 auto">Limpiar</a>
        @endif
    </form>
        ...
        <table class="data-table">
            <thead>
                <tr>
                    ...
                    <th>Acción</th>
                    <th>Módulo</th>
                    <th class="col-hide-mobile">Detalle</th>
            ...
            <tbody>
                ...
                <tr class="hoverable" style="cursor:pointer" onclick="window.location='{{ route('historial.show', $l) }}'">
                    <td class="col-hide-mobile" style="color:var(--color-text-muted); font-family:monospace; font-size:11px">#{{ str_pad($l->id, 5, '0', STR_PAD_LEFT) }}</td>
                    <td>
                    ...
                    <td>{{ $l->modulo }}</td>
                    <td class="col-hide-mobile" style="color:var(--color-text-muted); font-size:12.5px">{{ $l->detalle }}</td>
                    <td class="col-hide-mobile" style="font-family:monospace; font-size:11px; color:var(--color-text-muted)">{{ $l->ip_address ?? '—' }}</td>
                    <td style="color:var(--color-text-muted); font-size:12px; white-space:nowrap">{{ $l->created_at->format('d/m/Y H:i') }}</td>
                </tr>
                @empty
                <tr>
